...
import br.com.codiub.ordemservicos.model.constant.TipoOsServicoEnum;
import br.com.codiub.ordemservicos.model.entity.OsServico;
import br.com.codiub.ordemservicos.model.entity.OsServico_;
import br.com.codiub.ordemservicos.model.filter.OsServicoFilter;

public class OsServicoSpec {

    private OsServicoSpec() {}

    public static Specification<OsServico> usingFilter(OsServicoFilter filter, Pageable pageable) 
{
...
(OsServico_.ID), filter.getId()));
			}

            if (filter.getNome() != null && !filter.getNome().isEmpty()) {
                predicateList.add(builder.like(builder.upper(root.get(OsServico_.NOME)),
                    "%" + filter.getNome().toUpperCase() + "%"));
            }

            if (filter.getTipoPessoa() != null && !filter.getTipoPessoa().isEmpty()) {
                predicateList.add(builder.equal(
                    builder.upper(root.get(OsServico_.TIPO_PESSOA)),
