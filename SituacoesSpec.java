...
import br.com.codiub.ordemservicos.model.constant.TipoSituacoesEnum;
import br.com.codiub.ordemservicos.model.entity.Situacoes;
import br.com.codiub.ordemservicos.model.entity.Situacoes_;
import br.com.codiub.ordemservicos.model.filter.SituacoesFilter;

public class SituacoesSpec {

    private SituacoesSpec() {}

    public static Specification<Situacoes> usingFilter(SituacoesFilter filter, Pageable pageable) 
{
...
(Situacoes_.ID), filter.getId()));
			}

            if (filter.getNome() != null && !filter.getNome().isEmpty()) {
                predicateList.add(builder.like(builder.upper(root.get(Situacoes_.NOME)),
                    "%" + filter.getNome().toUpperCase() + "%"));
            }

            if (filter.getTipoPessoa() != null && !filter.getTipoPessoa().isEmpty()) {
                predicateList.add(builder.equal(
                    builder.upper(root.get(Situacoes_.TIPO_PESSOA)),
