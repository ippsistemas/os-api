...
import br.com.codiub.ordemservicos.model.constant.TipoVeiculosEnum;
import br.com.codiub.ordemservicos.model.entity.Veiculos;
import br.com.codiub.ordemservicos.model.entity.Veiculos_;
import br.com.codiub.ordemservicos.model.filter.VeiculosFilter;

public class VeiculosSpec {

    private VeiculosSpec() {}

    public static Specification<Veiculos> usingFilter(VeiculosFilter filter, Pageable pageable) 
{
...
(Veiculos_.ID), filter.getId()));
			}

            if (filter.getNome() != null && !filter.getNome().isEmpty()) {
                predicateList.add(builder.like(builder.upper(root.get(Veiculos_.NOME)),
                    "%" + filter.getNome().toUpperCase() + "%"));
            }

            if (filter.getTipoPessoa() != null && !filter.getTipoPessoa().isEmpty()) {
                predicateList.add(builder.equal(
                    builder.upper(root.get(Veiculos_.TIPO_PESSOA)),
